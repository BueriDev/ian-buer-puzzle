...
export default class Board extends React.PureComponent {
  ...
  handleTouchMove(square, index, { top, left }) {
    ...
    const { top: initialTop, left: initalLeft} = calculateItemPosition(size, index);
    ...
    this.animatedValues[square].left.setValue(initalLeft + clampedLeft);
  ...
  async componentDidUpdate(prevProps) {
    const { previousMove, onTransitionOut, puzzle, teardown } = this.props;

    const didMovePiece = prevProps.puzzle !== puzzle && previousMove !== null;
    const shouldTeardown = !prevProps.teardown && teardown;
